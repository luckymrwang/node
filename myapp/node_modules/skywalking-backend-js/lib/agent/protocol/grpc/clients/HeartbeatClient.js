...
var HeartbeatClient = /** @class */ (function () {
    function HeartbeatClient() {
        this.managementServiceClient = new Management_grpc_pb_1.ManagementServiceClient(AgentConfig_1.default.collectorAddress, grpc.credentials.createInsecure(), {
            interceptors: [AuthInterceptor_1.default],
        });
    }
    Object.defineProperty(HeartbeatClient.prototype, "isConnected", {
        get: function () {
            return this.managementServiceClient.getChannel().getConnectivityState(true) === grpc_1.connectivityState.READY;
        },
        enumerable: false,
        configurable: true
    });
    HeartbeatClient.prototype.start = function () {
        ...
        var instanceProperties = new Management_pb_1.InstanceProperties()
            ...
            new Common_pb_1.KeyStringValuePair().setValue('hostname').setValue(os.hostname()),
            new Common_pb_1.KeyStringValuePair().setValue('Process No.').setValue("" + process.pid),
        ]);
        this.heartbeatTimer = setInterval(function () {
            _this.managementServiceClient.reportInstanceProperties(instanceProperties, function (error, _) {
                if (error) {
                    logger.error('Failed to send heartbeat', error);
                }
            });
            _this.managementServiceClient.keepAlive(keepAlivePkg, function (error, _) {
                if (error) {
                    logger.error('Failed to send heartbeat', error);
                }
            });
